...
{
    int tmp_cleft = c - cw; // 剩余重量
    int tmp_cp = cp;        // 当前价值
    // 将剩余物品按照其单位重量价值排序，然后依次装入物品，直至装不下时
...
// 加入活性队列
void AddAliveNode(priority_queue<MaxHeapQNode *, vector<MaxHeapQNode *>, cmp> &q,
                  MaxHeapQNode *E, int up, int wt, int curp, int i, int ch)
{
...
void MaxKnapsack(int n, int c, vector<int> &bestx, vector<Object> &obj)
{
    ...
    MaxHeapQNode *E = NULL;
    cw = cp = bestp = 0; // 当前重量，当前价值，最优值
    int i = 1;
    int up = Bound(1, n, c, obj); // Bound(i)函数计算的是i还未处理时候的上限值
    while (i != n + 1)
    {
        int wt = cw + obj[i].weight; // 选左孩子
        if (wt <= c)                 // 满足约束条件
        {
            if (bestp < cp + obj[i].price) // 更新最优值
                bestp = cp + obj[i].price;
            AddAliveNode(q, E, up, cw + obj[i].weight, cp + obj[i].price, i, 1); // 加入活结点队列
        }
        up = Bound(i + 1, n, c, obj); // 不选右孩子的话（i+1），最多能装多少
        // 注意这里 up != up - obj[i].price而且 up >= up - obj[i].price
        if (up >= bestp) // 注意这里必须是大于等于
        {
            AddAliveNode(q, E, up, cw, cp, i, 0); // 选择右孩子
        }
        E = q.top();
        q.pop(); // 取出已处理的节点
        cw = E->weight;
    ...
    for (int j = n; j > 0; --j)
    {
        bestx[obj[E->lev - 1].id] = E->lchild; // obj[E->lev - 1].id就是物品标号，统计被选择的物品
        E = E->parent;                         // 从下往上遍历
    }
}
